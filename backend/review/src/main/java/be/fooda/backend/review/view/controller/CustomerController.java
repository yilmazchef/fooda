package be.fooda.backend.review.view.controller;

import be.fooda.backend.review.dao.CustomerRepository;
...
import be.fooda.backend.review.model.http.HttpEndpoints;
import be.fooda.backend.review.model.http.HttpFailureMessages;
import be.fooda.backend.review.model.http.HttpSuccessMessages;
import be.fooda.backend.review.service.mapper.CustomerMapper;
...
@RestController
@RequestMapping("/api/v2/review/customer")
...
public class CustomerController {
    ...
    public ResponseEntity<CustomerResponse> create(@RequestBody @NotNull CreateCustomerRequest request) {
        ...
        if (customerRepository.existsBySession(request.getSession())) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, HttpFailureMessages.CUSTOMER_REVIEW_EXIST_CANNOT_BE_CREATED.getDescription());
        }
    ...
    public ResponseEntity<CustomerResponse> updateById(@RequestParam("customerId") @NotNull String customerId,
        ...
        return customerRepository
                ...
                .orElseThrow(() -> {
                    throw new ResponseStatusException(HttpStatus.ACCEPTED, HttpFailureMessages.CUSTOMER_REVIEW_EXIST_CANNOT_BE_UPDATED.getDescription());
                });
    }
        ...
        return customerRepository
                ...
                .map(entity -> {
                    ...
                    return ResponseEntity
                            ...
                            .body(HttpSuccessMessages.CUSTOMER_REVIEW_DELETED.getDescription());
                })
                .orElseThrow(() -> {
                    throw new ResponseStatusException(HttpStatus.NOT_FOUND, HttpFailureMessages.CUSTOMER_REVIEW_NOT_FOUND.getDescription());
                });
    }
    ...
    // SEARCH
    @GetMapping(HttpEndpoints.GET_SEARCH_TEXT)
    public ResponseEntity<?> search(@RequestParam(required = true) String review,
        ...
        return ResponseEntity
                ...
                .body(
                        customerRepository
                                .findAllByReviewContaining(review, PageRequest.of(pageNo - 1, pageSize))
                                .stream()
                                .map(customerDocument -> customerMapper.toResponse(customerDocument))
                                .collect(Collectors.toUnmodifiableList())
                );
    ...
    public ResponseEntity<String> existsById(@RequestParam("customerId") @NotNull String customerId) {
        ...
        return customerRepository.existsById(new String(customerId))
                ? ResponseEntity.status(HttpStatus.FOUND).body(HttpSuccessMessages.CUSTOMER_REVIEW_EXISTS.getDescription())
                : ResponseEntity.status(HttpStatus.NOT_FOUND).body(HttpFailureMessages.CUSTOMER_REVIEW_DOES_NOT_EXIST.getDescription());
    }
